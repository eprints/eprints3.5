...
client_side_hashing = true; // Whether we should hash the file on the client to compare with the server generated hash
...
class Screen_EPrint_UploadMethod_File {
	 ...
	 */
	drop (evt) {
		const files_dropped = [...evt.dataTransfer.items]
		...
		this.handleFiles(files_dropped);
	}
...
const UploadMethod_process_file = (form, formData, prefix, component, fileLabel) => {
	...
	_xhr.addEventListener('load', function () {
		...
		if (response['docid']) {
			Component_Documents.instances.map((instance) => instance.refresh_document(response['docid']));
		}

		if (response['messages']) {
			const messages = document.createRange().createContextualFragment(response['messages']);
			document.getElementById('ep_messages').replaceWith(messages);
		} else {
			document.getElementById('ep_messages').innerText = '';
		}
	});
	...
	if (client_side_hashing) {
		progressContainer.progress_info.textContent = 'Hashing...';
		UploadMethod_hash_file(formData.get(`${prefix}_file`)).then(
			(fileHash) => {
				formData.set('file_hash', fileHash);
				_xhr.send(formData);
			}
		);
	} else {
...
async function UploadMethod_hash_file(file) {
	/*
	Previously we used to only have one fileHasher object, now we have one per file.
	Possibly this is less efficient, but it avoids issues when multiple files are uploaded at once
	*/
	const fileHasher = await hashwasm.createMD5();
	...
	const chunkCount = Math.floor(file.size / hashChunkSize);
	for (let i = 0; i <= chunkCount; i++) {
		const chunk = file.slice(hashChunkSize * i, Math.min(hashChunkSize * (i + 1), file.size));
		await hashChunk(chunk);
	}

	return fileHasher.digest();
}
