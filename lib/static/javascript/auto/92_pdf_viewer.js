window.onload = () => {
	...
	{
		let pdfElem = pdfWrapperElem.querySelector(".ep_document_preview_pdf")
		let iframeElem = pdfWrapperElem.querySelector(".ep_document_preview_pdf iframe")
		...
		pdfButtonElem.addEventListener('click', () => {
			...
			iframeElem.style.height = ( window.innerHeight * 0.75 ) + "px"
		...
		pdfButtonElemToggle.addEventListener("click", () => {
			...
			if( pdfButtonToggleClose.classList.contains('d-none') ) {
				...
				iframeElem.style.height = ( window.innerHeight * 0.40 ) + "px"
			}
		})
	}
}
