...
function Component_Field_Action(e, inputName, inputValue, prefix) {
	...
	xhrRequest(url, {
		...
		onSuccess: ((response) => {
			...
			// Re-apply the 'simple view', if set, to simplify down the fresh table
			if (prefix == "c_contributions") {
				let current_viewmode = document.getElementById("contributor_viewmode_button").getAttribute("data-viewmode");
				if (current_viewmode == "simple") {
					switchContribView(enable_simple_view = true);
				}
			}
