 ...
 *
 * @readme https://github.com/eprints/eprints3.5
 *
 * @maintainer EPrints Services
 *
 */
 ...
 *
 * @param {DOM Element} element The element being manipulated for the effect.
 * @param {number} startPosition Where relative to the elements position the animation should start.
 * @param {number} newPosition Where relative to the elements position the animation should end.
 * @param {number} duration Duration of the effect in milliseconds.
 * @param {number} steps Resolution of the effect.
 * @param {function} callback Executed at the end of the effect.
 * @return {undefined} n/a Void function
 ...
 *
 * @param {DOM Element} element The element whose attributes are being transposed into a hash.
...
/**
 * Replacing Prototype's autocompleter for the metafields in the eprints workflow.
 *
			...
			onSuccess: (response) => {
				...
				lis.forEach((li) => {
					...
					li.addEventListener('click', (e) => {
						...
						let lookup_button = div_to_populate.parentElement.parentElement.querySelector('input[type="submit"]');
						if (lookup_button)
						{
							lookup_button.click();
						}
