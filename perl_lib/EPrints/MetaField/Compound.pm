...
sub validate
{
	...
	# This will validate the individual sub-fields e.g. creators_name, creators_id, etc.
	foreach my $field_conf ( @{$f} )
	{
		push @problems, $object->validate_field( $field_conf->{name} );
	}

	# This can validate the compound field as a whole - e.g. for a creator, does a 'row' that has an ID defined also have a name defined
